...
awk_cond2 = (dim eq "x") ? "($1 - val) * ($1 - val) < tol*tol {print $2, $9}" : "($2 - val) * ($2 - val) < tol*tol {print $1, $9}"
...
# If slicing at constant x: filter by col1 and print y,theta ($2,$3)
# If slicing at constant y: filter by col2 and print x,theta ($1,$3)
awk_cond = (dim eq "x") ? "($1 - val) * ($1 - val) < tol*tol {print $2, $11}" : "($2 - val) * ($2 - val) < tol*tol {print $1, $11}"
awk_cond2 = (dim eq "x") ? "($1 - val) * ($1 - val) < tol*tol {print $2, $12}" : "($2 - val) * ($2 - val) < tol*tol {print $1, $12}"
