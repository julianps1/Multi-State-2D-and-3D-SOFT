...
C   System:          NaFH
C   Common name:     NaFH surface fit B
C   Functional form: Modified Extended LEPS with a LEPS correction function.
...
C   Notes:  This is an improved version of NaFH surface fit A.  The diabatic 
C           coupling has been made to vanish asymptotically and the diatomic 
C           curves have been replaced with experimental curves.
C
C   Protocol:
...
C
C
C  This is version 2 of the NaFH potential energy matrix.
C  Details can be found in 
C
C  "Coupled Quasidiabatic Potential Energy Surfaces for LiFH", 
C  M. D. Hack, A. W. Jasper, D. G. Truhlar, P. Piecuch, prepared 
C  for publication in J. Chem. Phys.
c
C  This surface is distributed with the NAT code.
C
c  This surface uses version 1, but the diagonal diabats are corrected with
c  a correction function which is the difference of two LEPS functions.  The
c  surfaces are corrected such that they have the experimental bond energies.
c  The diabatic coupling is cut off in the HF bond direction so that it vanishes 
c  asymptotically.
...
c **********************************************************************
      ...
      write(6,100)
      write(6,*)'PREPOT called for the NaFH version 2.0 ',
     &                                 'potential energy surface.'
      write(6,*)'Last modified August 31, 2000.'
      write(6,*)'This version has derivatives.'
      write(6,*)'Contains modified switching function.'
      endif
         call prepot12
         call prepot22
         call prepot11
      if( g_myrank .eq. 0 )write(6,100)
...
C               that
C               R(i,1) should be the distance between atoms 1 and 2 (A-B), NaH
C               R(i,2) should be the distance between atoms 2 and 3 (B-C), HF
C               R(i,3) should be the distance between atoms 1 and 3 (A-C), NaF
...
C----------------------------------
C     Excited NaF surface:
c        write(6,102)(cNF1(i),i=1,5)
C----------------------------------
C     Ground HF suface:
c        write(6,103)DeHF,reHF,(cHF(i),i=1,6)
C----------------------------------
c        write(6,104)ads1,ars1,abs1
...
c        write(6,121)(dip1(j),j=1,4)
c        write(6,122)(adip1(j),j=1,4)
c        write(6,123)(rdip1(j),j=1,4)
c        write(6,124)(acut1(j),j=1,4)
c        write(6,125)(rcut1(j),j=1,4)
c        write(6,126)(dipn(j),j=1,4)
c        write(6,127)(al2(j),j=1,4)
c        write(6,128)(aln0(j),j=1,4)
c        write(6,129)(dipnn(j),j=1,4)
c        write(6,130)(al2n(j),j=1,4)
c        write(6,131)(aln0n1(j),j=1,4)
c        write(6,132)(aln0n2(j),j=1,4)
c        write(6,133)(rn0n(j),j=1,4)
      endif
      return
c
